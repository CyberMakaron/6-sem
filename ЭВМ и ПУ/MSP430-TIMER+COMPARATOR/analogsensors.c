...
word res_measure_TB(byte Rpin){
  ...
  TBCTL &= ~(CNTL0 + CNTL1);    // CNTL0,CNTL1 = 00 - 16-разрядный таймер-счётчик
  // тактирование от SMCLK, делитель /4, очистка счетчика, непрерывный режим счета
...
word R22_get_resistance_TB(){
  P2SEL &= ~(Rref+Rx);            // функция ввода-вывода для ножек P2.4 и P2.5
  word Nref = res_measure_TB(Rref);  // время разряда через опорный резистор
  word Nx = res_measure_TB(Rx);      // время разряда через подстроечный резистор
  return ((100000*Nx)/Nref)-10000; // R22 = (100000 * Nx / Nref) - 10000
}

// обработчик прерываний от таймера B
